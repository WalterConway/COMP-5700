...
class PythonScript(object):
    ...
    def extractDesign(self):
        ...
        if(len(designList[0]) != 0):
            designList[0].sort(key=lambda item:item.componentName)
        if(len(designList[1]) != 0):
            designList[1].sort(key=lambda item:item.componentName)
...
class Line(object):
    ...
    def isStringComment(self, LineFromPythonFile):
        tempLine = LineFromPythonFile.strip()
        if(tempLine.startswith('"') and tempLine.endswith('"')):
            return True
        return False

    def isOneLineDocString(self, LineFromPythonFile):
        tempLine = LineFromPythonFile.strip()
        if(tempLine.startswith("'") and tempLine.endswith("'") and not tempLine.startswith("'''")):
            return True
        return False
        ...
        while(True):
            ...
            if(line != None):
                ...
                if(self.pythonLine.isDocString(line)):
                    continue
                if(self.pythonLine.isStringComment(line)):
                    continue
                if(self.pythonLine.isOneLineDocString(line)):
                    continue
